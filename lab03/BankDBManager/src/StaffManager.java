		...
		if (e.getSource() == updateBtn) {
			...
			for (int i = 0; i < rowCnt; i ++) {
				...
				for (int j = 0; j < colCnt - 1; j ++) {
					if (j == 5) {
						cond += colName[j + 1] + " = " + "to_date('" + oldTable[i][j + 1].toString().substring(0, 10) + "', 'yyyy/mm/dd')";
						change += colName[j + 1] + " = " + "to_date('" + resTable.getValueAt(i, j + 1).toString().substring(0, 10) + "', 'yyyy/mm/dd')";
					}
					else if (oldTable[i][j + 1] == null || oldTable[i][j + 1].equals("")) {
						cond += colName[j + 1] + " is null ";
					}
					else {
						cond += colName[j + 1] + " = '" + oldTable[i][j + 1] + "' ";
						change += colName[j + 1] + " = '" + resTable.getValueAt(i, j + 1) + "' ";
					}
					if (j < colCnt - 2) {
						cond += " and ";
						change += ",";
